"""
WebSocket proxy that runs in the bridge container.
Allows browser JavaScript to connect to localhost:8766,
which then forwards to wormhole-server:8765.
"""
import asyncio
import websockets
import os
...
async def proxy_handler(client_websocket):
    """Handle incoming connection from browser and proxy to wormhole-server"""
    ...
    try:
        # Connect to the actual wormhole server
...
async def main():
    ...
    print(f"[Proxy] Forwarding to {os.getenv('wormhole_server_host', 'wormhole-server')}:{os.getenv('wormhole_port', 8765)}")
