(function() {
    ...
    // Predefined command handlers to avoid CSP eval restrictions
    const commandHandlers = {
        createConversation: async (params) => {
            ...
            const messageResponse = await fetch(baseUrl + '/conversations/' + conversation.id + '/turn-streaming', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-Token': csrfToken,
            ...
            return { success: true, conversationId: conversation.id, response: fullResponse };
        },
        ...
        sendMessage: async (params) => {
            ...
            const messageResponse = await fetch(baseUrl + '/conversations/' + conversationId + '/turn-streaming', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-Token': csrfToken,
        ...
        ws.onmessage = async (event) => {
            ...
            try {
                ...
                if (message.command && commandHandlers[message.command]) {
                    try {
                        const result = await commandHandlers[message.command](message.params || {});
