...
agent_workflow = AgentWorkflow(lambda: active_conversation_id, lambda cid: set_active_conversation(cid), lambda cid, p, s, r: log_to_data_folder(cid, p, s, r))
...
async def list_models():
    ...
    return {
        "object": "list",
        "data": [
            {"id": "gpt-5-chat", "object": "model", "created": 1730419200, "owned_by": "openai"},
            {"id": "gpt-5-2025-08-07", "object": "model", "created": 1730419200, "owned_by": "openai"},
            {"id": "GPT-4o", "object": "model", "created": 1715367600, "owned_by": "openai"},
            {"id": "gpt-4o-audio-preview-2025-06-03", "object": "model", "created": 1730419200, "owned_by": "openai"},
            {"id": "gpt-4o-mini-audio-preview-2024-12-17", "object": "model", "created": 1730419200, "owned_by": "openai"},
            {"id": "GPT-4.1", "object": "model", "created": 1715367600, "owned_by": "openai"},
            {"id": "o3", "object": "model", "created": 1730419200, "owned_by": "openai"},
            {"id": "o4-mini", "object": "model", "created": 1730419200, "owned_by": "openai"},
            {"id": "claude-sonnet-4-5-20250929", "object": "model", "created": 1730419200, "owned_by": "anthropic"},
            {"id": "claude-haiku-4-5-20251001", "object": "model", "created": 1730419200, "owned_by": "anthropic"},
            {"id": "claude-opus-4-1-20250805", "object": "model", "created": 1730419200, "owned_by": "anthropic"},
            {"id": "claude-opus-4-20250514", "object": "model", "created": 1730419200, "owned_by": "anthropic"},
            {"id": "claude-sonnet-4-20250514", "object": "model", "created": 1730419200, "owned_by": "anthropic"},
            {"id": "gemini-2.5-pro-preview-06-05", "object": "model", "created": 1730419200, "owned_by": "google"},
            {"id": "gemini-2.5-flash-preview-05-20", "object": "model", "created": 1730419200, "owned_by": "google"},
            {"id": "Grok 3", "object": "model", "created": 1730419200, "owned_by": "xai"},
            {"id": "Llama 4 Maverick", "object": "model", "created": 1730419200, "owned_by": "meta"},
            {"id": "qwen3-235b-a22b-2507-v1", "object": "model", "created": 1730419200, "owned_by": "alibaba"},
            {"id": "deepseek-r1-0528", "object": "model", "created": 1730419200, "owned_by": "deepseek"}
        ]
    }
...
async def chat_completions(request: Request):
    ...
    if not model:
        return {"error": {"message": "Model is required", "type": "invalid_request_error", "code": "model_required"}}, 400
    ...
    for i, msg in enumerate(messages):
        ...
        if role == "system":
            ...
            attachments_match = re.search(r'<attachments>(.*?)</attachments>', raw_user, re.DOTALL)
            if attachments_match:
                attachments = attachments_match.group(0)
            
            user_request_match = re.search(r'<userRequest>(.*?)</userRequest>', raw_user, re.DOTALL)
    ...
    if raw_system or raw_user:
        ...
        print(f"[Dump] Saved raw prompts - system: {len(raw_system)} chars, user: {len(raw_user)} chars")
    else:
    ...
    if is_new_conversation:
        global active_conversation_id
        active_conversation_id = None
        print(f"[VSCode] New conversation detected (no assistant messages, total messages: {len(messages)})")
    else:
        print(f"[VSCode] Continuing conversation (has assistant messages, total messages: {len(messages)})")
    ...
    if tools and (not has_tool_results or last_assistant_had_final_answer):
        ...
        if conversation_id is None:
            return {"error": {"message": "Failed to create conversation", "type": "server_error"}}, 500
    else:
        if has_tool_results and not last_assistant_had_final_answer:
            ...
            if conversation_id is None:
                return {"error": {"message": "Failed to get response from Outlier", "type": "server_error"}}, 500
    ...
    if stream:
        ...
        prompt_tokens = sum(len(msg.get("content", "").split()) for msg in messages if msg.get("content"))
